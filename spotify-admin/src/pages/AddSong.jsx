...
const AddSong = () => {
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      // FormData is a built-in JavaScript object that provides a way to construct a set of key/value pairs representing form fields and their values. It is primarily used to send form data, especially file uploads, via XMLHttpRequest or fetch API.

      // Key Features of FormData:
      // Ease of Use: Simplifies the process of sending data from a form, including files.
      // File Uploads: Supports uploading files with other form fields.
      // Compatibility: Works with both XMLHttpRequest and fetch.
      ...
      const response = await axios.post(`${url}/api/song/add`, formData) //use axios to call api
    ...
    <form onSubmit={onSubmitHandler} className='flex flex-col items-start gap-8 text-gray-800'>
      ...
      <div className='flex flex-col gap-2.5'>
        ...
        <select onChange={(e)=>setAlbum(e.target.value)} defaultValue={album} className='bg-transparent outline-green-600 border-2 border-gray-400 p-2.5 w-[150px]' >
          ...
          {/* {albumData.map((item, index) => { return (
